...
(() => {
  ...
  function buildPage(page) {
    ...
    page.innerHTML = `
      <div class="page-header">
        <div>
          <span class="page-label">ANALYSE</span>
          <h2>Scanner le Drawing</h2>
          <p>Importez un Drawing réel puis analysez-le avec MUSTER BAW Vision AI.</p>
        </div>
      </div>
      <div class="analyzer-shell">
        <div class="dashboard-card">
          <div class="analyzer-dropzone" id="drawingDropzone">
            ...
            <p>PDF, PNG, JPG, WEBP, BMP ou TIFF — maximum 50 MB.</p>
            <label class="btn btn-primary" for="drawingFile"><i class="fa-solid fa-file-import"></i> Choisir un fichier</label>
            <input id="drawingFile" type="file" accept="application/pdf,image/png,image/jpeg,image/webp,image/bmp,image/tiff" hidden>
          </div>
          <div class="analyzer-progress" id="analyzerProgress">
            <strong id="analyzerProgressLabel">Prêt</strong>
            <div class="analyzer-progress-bar"><span id="analyzerProgressBar"></span></div>
            <span id="analyzerPercent">0%</span>
          </div>
        </div>
  ...
  function valid(file) {
    ...
    return file.type === "application/pdf" || /^(image\/(png|jpeg|webp|bmp|tiff))$/i.test(file.type) || /\.(pdf|png|jpe?g|webp|bmp|tiff)$/i.test(file.name);
  }
  ...
  function loadFile(file) {
    ...
    currentFile = file;
    window.MusterDrawingFile = file;
    localStorage.setItem("musterLastDrawingFile", file.name);
    setProgress("Drawing chargé — aperçu prêt", 100);
    renderPreview(file);
    ensureAiCard();
    toast(`Drawing chargé : ${file.name}`, "success");
  }
  ...
  function renderPreview(file) {
    ...
    root.innerHTML = `
      <div class="dashboard-card stable-preview-card">
        <div class="card-header">
          <div><h3><i class="fa-solid fa-image"></i> Prévisualisation du Drawing</h3><p>${esc(file.name)} · ${bytes(file.size)}</p></div>
          <span class="ai-status"><span></span> Fichier prêt</span>
        </div>
        <div class="stable-preview-frame" id="drawingPreviewFrame"></div>
      </div>`;
    ...
    if (file.type === "application/pdf" || /\.pdf$/i.test(file.name)) {
      ...
      img.src = previewUrl; img.alt = "Drawing chargé"; img.loading = "eager";
  ...
  function ensureAiCard() {
    ...
    if ($("#musterVisionCard", page)) return;
    const card = document.createElement("div");
    card.className = "dashboard-card ai-vision-card";
    card.id = "musterVisionCard";
    card.innerHTML = `
      <div class="card-header">
        <div><h3><i class="fa-solid fa-brain"></i> MUSTER BAW Vision AI</h3><p>Analyse visuelle du Drawing : fils, couleurs, connecteurs, pins et relations.</p></div>
        <button class="btn btn-small" id="aiKeyBtn"><i class="fa-solid fa-key"></i> Configurer IA</button>
      </div>
      <div class="ai-vision-actions">
        <button class="btn btn-primary" id="runAiDrawing"><i class="fa-solid fa-wand-magic-sparkles"></i> Analyser avec l'IA</button>
        <span id="aiStatus">Drawing prêt.</span>
      </div>
  ...
  async function runAi() {
    const file = currentFile || window.MusterDrawingFile || $("#drawingFile")?.files?.[0];
    if (!file) return toast("Choisissez d'abord un Drawing.", "warning");
    const status = $("#aiStatus"), out = $("#aiResults"), button = $("#runAiDrawing");
    if (button) button.disabled = true;
    if (status) status.textContent = "Connexion à Vision AI…";
    try {
      if (window.MusterStableVision?.run) await window.MusterStableVision.run();
        ...
        if (window.MusterStableVision?.run) await window.MusterStableVision.run();
        else throw new Error("Le module Vision AI n'est pas encore chargé. Actualisez la page et réessayez.");
      }
    } catch (e) {
      console.error(e);
      if (out) out.innerHTML = `<div class="ai-error"><strong>Analyse non lancée</strong><p>${esc(e.message)}</p><button class="btn btn-small" id="retryVision"><i class="fa-solid fa-rotate-right"></i> Réessayer</button></div>`;
      toast("Impossible de lancer Vision AI.", "error");
      $("#retryVision")?.addEventListener("click", runAi);
  ...
  function observe() {
    ...
    mountedPage = page;
    buildPage(page);
    // The page is now the single owner of the file input; legacy OCR is never started automatically.
  }
