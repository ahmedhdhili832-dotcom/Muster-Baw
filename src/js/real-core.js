...
(() => {
  ...
  const state = read(STORE, {
    projects: [],
    analyses: [],
    settings: { company: "SEBN", environment: "PPE" }
  });

  function persist() { write(STORE, state); }

  function analysisSnapshot() {
    const a = read(ANALYSIS, null);
    if (!a) return null;
    return {
      id: `ANA-${Date.now()}`,
      file: a.file || "Drawing",
      ...
      pins: Array.isArray(a.pins) ? a.pins.length : 0,
      lines: Array.isArray(a.geometry?.lines) ? a.geometry.lines.length : 0,
      junctions: Array.isArray(a.geometry?.junctions) ? a.geometry.junctions.length : 0,
  ...
  function ingestLatestAnalysis() {
    ...
    if (!exists) {
      state.analyses.push(latest);
      persist();
    }
  }

  function setMetric(label, value) {
    const candidates = $$(`.stat-card`).filter(card => card.textContent.includes(label));
    candidates.forEach(card => {
      const strong = $(".stat-card-content strong", card);
  ...
  function refreshDashboard() {
    ...
    const body = $("#page-dashboard .data-table tbody");
    if (!body) return;
    body.innerHTML = m.latest.length ? m.latest.map(a => `
      <tr>
        <td><strong>${escapeHtml(a.file)}</strong><small>${escapeHtml(a.id)}</small></td>
        <td>${escapeHtml(a.file)}</td>
        <td>${a.connectors}</td>
        <td>${a.wires}</td>
        <td>${a.confidence != null ? `${a.confidence} %` : "—"}</td>
        <td><span class="status-badge analysis">${escapeHtml(a.status)}</span></td>
      </tr>`).join("") : `<tr><td colspan="6">Aucune analyse réelle enregistrée. Importez un Drawing pour commencer.</td></tr>`;
  }
  ...
  function projectForm() {
    const existing = $("#realProjectModal");
    if (existing) existing.remove();
    const root = document.createElement("div");
    root.id = "realProjectModal";
    ...
    root.querySelector("#saveRealProject")?.addEventListener("click", () => {
      const name = $("#realProjectName")?.value.trim();
      const ref = $("#realProjectRef")?.value.trim();
      if (!name || !ref) return window.Toast?.show?.("Nom et référence requis.", "warning");
      state.projects.push({ id: ref, name, createdAt: new Date().toISOString() });
      persist();
      root.remove();
      window.Toast?.show?.(`Projet ${ref} créé.`);
      if (window.Navigation?.go) window.Navigation.go("projects"); else location.hash = "#projects";
    });
  }
  ...
  function bindRealActions() {
    document.addEventListener("click", e => {
      ...
      if (!action) return;
      if (action.dataset.action === "new-project") projectForm();
    });
  }
